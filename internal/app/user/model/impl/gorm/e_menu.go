...
//GetMenuDB 获取菜单存储
func GetMenuDB(ctx context.Context, defDB *DB) *DB {
	return getDBWithModel(ctx, defDB, Menu{})
}

// GetMenuActionDB 获取菜单动作存储
func GetMenuActionDB(ctx context.Context, defDB *DB) *DB {
	return getDBWithModel(ctx, defDB, MenuAction{})
}

// GetMenuResourceDB 获取菜单资源存储
func GetMenuResourceDB(ctx context.Context, defDB *DB) *DB {
...
type Menu struct {
	...
	Hidden     *int64  `gorm:"column:hidden;index;"`               // 隐藏菜单(0:不隐藏 1:隐藏)
...
func (a Menu) ToSchemaMenu() *user.MenuSchema {
	ts, _ := ptypes.TimestampProto(a.CreatedAt)
	item := &user.MenuSchema{
		RecordID:   a.RecordID,
		Name:       *a.Name,
		Sequence:   *a.Sequence,
		Icon:       *a.Icon,
...
func (a Menus) ToSchemaMenus() []*user.MenuSchema {
	list := make([]*user.MenuSchema, len(a))
	for i, item := range a {
...
type SchemaMenuAction user.MenuAction
...
func (a MenuAction) ToSchemaMenuAction() *user.MenuAction {
	return &user.MenuAction{
		Code: a.Code,
		Name: a.Name,
	}
}
...
func (a MenuActions) GetByMenuID(menuID string) []*user.MenuAction {
	var list []*user.MenuAction
	for _, item := range a {
...
func (a MenuActions) ToSchemaMenuActions() []*user.MenuAction {
	list := make([]*user.MenuAction, len(a))
	for i, item := range a {
...
type SchemaMenuResource user.MenuResource
...
func (a MenuResource) ToSchemaMenuResource() *user.MenuResource {
	return &user.MenuResource{
		Code:   a.Code,
		Name:   a.Name,
		Method: a.Method,
		Path:   a.Path,
...
func (a MenuResources) GetByMenuID(menuID string) []*user.MenuResource {
	var list []*user.MenuResource
	for _, item := range a {
		if item.MenuID == menuID {
...
func (a MenuResources) ToSchemaMenuResources() []*user.MenuResource {
	list := make([]*user.MenuResource, len(a))
	for i, item := range a {
